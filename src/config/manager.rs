...
impl ConfigLoader {
    ...
    pub fn load_config() -> Result<GrumpyConfig, ConfigError> {
        ...
        match toml::from_str::<GrumpyConfig>(&content) {
            Ok(config) => Ok(config),
            Err(_) => {
                eprintln!(
    ...
    fn create_default_config(path: &Path) -> Result<GrumpyConfig, ConfigError> {
        let default_config = GrumpyConfig::default();
